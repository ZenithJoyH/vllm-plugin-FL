...
def sparse_attn_indexer_kpool(
    ...
    fp8_dtype = INDEXER_BACKEND.query_dtype
            ...
            if n_prefill > 0:
                ...
                if tail_kv_cache is not None and tail_prefix is not None:
                    ...
                    if tail_meta is not None:
                        ...
                        INDEXER_BACKEND.persist_prefill_tail(
                            k[prefill_slice],
                            gate_score[prefill_slice],
        ...
        for chunk in prefill_metadata.chunks if not short_prefill else ():
            ...
            if index_kpool > 1:
                ...
                if positions is not None:
                    ...
                    expanded = INDEXER_BACKEND.append_tail_to_topk(
    ...
    if has_decode:
        ...
        if (
            ...
            if per_req_lens is not None:
                ...
                # in vLLM 0.24 does not carry the GLM5-Next extension fields,
        ...
        if index_kpool > 1:
            pool_ids = pool_topk.to(torch.int64)
            valid = pool_ids >= 0
            out = INDEXER_BACKEND.expand_pools_to_tokens(
                pool_ids, valid, topk_tokens, index_kpool
            )
            ...
            out = INDEXER_BACKEND.append_tail_to_topk(
...
class SparseAttnIndexerKpool(CustomOp):
    ...
    def __init__(
        ...
        INDEXER_BACKEND.validate_environment()
