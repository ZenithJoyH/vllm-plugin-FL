...
"""Transformers config classes for GLM-5.3-Flash.
...
class Glm5NextTextConfig(PretrainedConfig):
    ...
    def validate_layer_type(self):
        ...
        if len(self.layer_types) != self.num_hidden_layers:
            raise ValueError("GLM layer_types must have num_hidden_layers entries")
        if not set(self.layer_types) <= allowed:
            raise ValueError(f"Unsupported GLM layer_types: {self.layer_types}")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        glm_layer_types = self.layer_types
        # Transformers 5.8 validates inside its base-class initializer, using
        # its closed-over validator list. Present the equivalent generic type
        # during that validation, then retain the checkpoint's exact schema.
        self.layer_types = [
            "full_attention" if t == "deepseek_sparse_attention" else t
            for t in glm_layer_types
        ]
        super().__init__(
        ...
        self.layer_types = glm_layer_types
