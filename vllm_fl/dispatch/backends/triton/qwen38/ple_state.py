...
"""Stride-aware PLE state I/O owned by the portable Triton backend.

Accelerators use the same kernels in eager and graph mode. This avoids
materializing the multi-GiB transposed cache through FlagGems index_select and
requires neither a model-specific global blacklist nor CUDA capture detection.
CPU tensor paths are numerical references for focused tests.
"""
...
def _last_write_mask(indices: torch.Tensor) -> torch.Tensor:
    ...
    duplicate_later = (
        (indices.unsqueeze(-1) == indices.unsqueeze(0)).triu(diagonal=1).any(dim=1)
    )
...
def ple_state_scatter_(
    ...
    *,
    ...
    for row in range(safe_indices.numel()):
        ...
        selected = torch.where(effective_write_mask[row], rows[row : row + 1], existing)
