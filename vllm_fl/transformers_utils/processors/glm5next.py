...
Ports the GLM-5.3-Flash image/video preprocessing pipeline (``smart_resize`` +
Qwen-VL-style patchify + GLM-5.3-Flash dynamic-fps frame sampling) so vLLM no
...
are reused -- the GLM-5.3-Flash-specific logic lives here.
...
def smart_resize(
    ...
    """GLM-5.3-Flash ``smart_resize``: snap (h, w) to multiples of the spatial
    ...
    # The budget (max_pixels) is the TOTAL pixel budget for the whole clip
    # (shortest_edge/longest_edge from the checkpoint).  smart_resize shrinks
    # the SPATIAL size under that total budget; for long videos (large t) a
    # naive shrink can floor h_bar/w_bar to 0.  Guard: never go below the
    # smallest factor multiple.
    if t_bar * h_bar * w_bar > max_pixels:
        beta = math.sqrt((t * h * w) / max_pixels)
        h_bar = max(math.floor(h / beta / h_factor) * h_factor, h_factor)
        w_bar = max(math.floor(w / beta / w_factor) * w_factor, w_factor)
...
class Glm5NextVideoProcessor(BaseVideoProcessor):
    ...
    GLM-5.3-Flash's dynamic-fps frame sampling (``DYNAMIC_FPS_THRES``).
    """

    resample = PILImageResampling.BICUBIC
    size = {"shortest_edge": 112 * 112, "longest_edge": 28 * 28 * 2 * 30000}
    # ``size`` for video is the smart_resize PIXEL BUDGET (shortest_edge /
    # longest_edge are absolute pixel counts, not H/W).  The generic video
    # processor turns ``size`` into a SizeDict and then calls ``self.resize``
    # with that SizeDict, which requires H/W keys.  Provide a H/W default for
    # the generic ``self.resize`` so it never sees a SizeDict with only
    # shortest/longest_edge, and keep the real budget in the dedicated
    # ``_smart_resize_*`` fields used by smart_resize.
    size = {"shortest_edge": 112 * 112, "longest_edge": 28 * 28 * 2 * 30000}
    smart_resize_size = {
        "shortest_edge": 112 * 112,
        "longest_edge": 28 * 28 * 2 * 30000,
    }
    resize_size = {"height": 448, "width": 448}
    image_mean = OPENAI_CLIP_MEAN
    image_std = OPENAI_CLIP_STD
    do_resize = True
    do_rescale = True
    ...
    def __init__(self, **kwargs: Unpack[Glm5NextVideoProcessorKwargs]) -> None:
        # The GLM-5.3-Flash video processor's ``size`` carries the smart_resize
        # PIXEL BUDGET as shortest_edge/longest_edge (absolute pixel counts).
        # The generic TorchvisionBackend later passes that same ``size``
        # SizeDict to ``self.resize``, which needs height/width.  To keep both
        # callers happy: normalize the checkpoint's budget-style ``size`` into
        # a height/width form (448x448) for the generic path, and preserve the
        # real pixel budget for smart_resize in ``_smart_resize_*``.
        budget = kwargs.get("size") or {
            "shortest_edge": 112 * 112,
            "longest_edge": 28 * 28 * 2 * 30000,
        }
        if "height" not in budget and "width" not in budget:
            kwargs["size"] = {"height": 448, "width": 448}
        super().__init__(**kwargs)
        self._smart_resize_min = budget.get("shortest_edge")
        self._smart_resize_max = budget.get("longest_edge")
        if self._smart_resize_min is None or self._smart_resize_max is None:
            raise ValueError(
                "size must contain 'shortest_edge' and 'longest_edge' keys."
            )
    ...
    def _standardize_kwargs(self, **kwargs) -> dict:
        kwargs = super()._standardize_kwargs(**kwargs)
        # The generic path feeds ``size`` (now height/width after __init__
        # normalization) to ``self.resize``.  The smart_resize budget lives in
        # ``_smart_resize_*``, so no shortest/longest_edge requirement here.
        return kwargs
        ...
        for shape, stacked_videos in grouped_videos.items():
            ...
            if do_resize:
                resized_height, resized_width = smart_resize(
                    ...
                    h_factor=patch_size * merge_size * patch_expand_factor,
                    w_factor=patch_size * merge_size * patch_expand_factor,
                    min_pixels=self._smart_resize_min,
                    max_pixels=self._smart_resize_max,
                )
...
class Glm5NextProcessor(ProcessorMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        The GLM-5.3-Flash checkpoint stores its image/video processor configs inside
            ...
            for i in range(len(text)):
                while self.video_token in text[i]:
                    ...
                    if metadata.fps is None:
                        logger.warning_once(
                            "GLM-5.3-Flash requires frame timestamps to construct "
