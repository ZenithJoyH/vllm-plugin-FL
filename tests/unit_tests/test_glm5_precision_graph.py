...
def test_all_capture_shapes_keep_live_block_tables():
    ...
    assert len(owner._sparse_indexer_block_tables) == 6
...
def test_mhc_norm_eager_graph(batch):
    ...
    from vllm_fl.dispatch.backends.reference.impl.mhc import mhc_pre
...
def test_model_local_bounded_activation_graph():
    ...
    with policy_context(
        SelectionPolicy.from_dict(
            per_op_order={"silu_and_mul_with_clamp": ["flagos"]}, strict=True
        )
    ):
